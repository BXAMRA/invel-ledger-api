...
class DocumentController extends Controller
{
  ...
  public function index(Request $request): JsonResponse
  {
    ...
    if ($request->has("sort_by")) {
      ...
      if ($sortBy === "customer") {
        $query->join("customers", "documents.customer_id", "=", "customers.id")->select("documents.*")->orderBy("customers.company_name", $sortDir);
    ...
    $document = DB::transaction(function () use ($validated) {
      ...
      if (!$documentNumber) {
        ...
        if ($validated["type"] === "invoice") {
          $prefixSetting = Setting::query()->where("key", "invoice.prefix")->value("value");
          if ($prefixSetting) {
            $prefix = json_decode($prefixSetting, true) ?? $prefixSetting;
          }
          $digitsSetting = Setting::query()->where("key", "invoice.digits")->value("value");
          if ($digitsSetting) {
   ...
   */
  public function downloadAttachment(Document $document, int $index): JsonResponse
  {
    $attachments = $document->attachments ?? [];
